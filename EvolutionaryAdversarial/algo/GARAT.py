...
from EvolutionaryAdversarial.buffer import RolloutBuffer
from EvolutionaryAdversarial.network import StateIndependentPolicy, StateFunction
from torch.utils.tensorboard import SummaryWriter
from EvolutionaryAdversarial.network.disc import GARATDiscrim
...
class GARAT_PPO(Algorithm):
    ...
    def step(self, env, number_of_env, state, step):
        ...
        actual_action = 0.5 * origin_action + 0.5 * GAT_action
...
# 希望使修正后的A，形成的sa(源a)s'，和真实世界的sas'更加相似。
    def __init__(self, buffer_exp, state_shape, action_shape, number_of_envs,
                 ...
                 units_disc=(100, 100), epoch_ppo=10, epoch_disc=30,
    ...
    def update(self, writer):
        ...
        for _ in range(self.epoch_disc):
            ...
            # states_exp, actions_exp, _, _, _,  next_states_exp = self.buffer.sample(self.batch_size)
            # states_exp = states_exp[:, :self.state_shape[0]]
            # actions_exp = actions_exp[:, -self.action_shape[0]:]
            # next_states_exp = next_states_exp[:, :self.state_shape[0]]
        ...
        writer.add_scalar('reward/imit_rewartd', rewards.mean(), self.learning_steps)
        # print(self.learning_steps,'imit_reward:',rewards.mean())
