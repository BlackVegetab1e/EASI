...
class DR_Trainer:
    ...
    def __init__(self, env, number_of_env, algo, log_dir,  sim_params_1, sim_params_2, DR_Type = None, seed=0, num_steps=10**5,
                 eval_interval=10**3, num_eval_steps=20000, GAT_policy = None):
        super().__init__()
    ...
    def train(self):
        ...
        while isaac_gym_sum_step < self.num_steps:
            ...
            # Pass to the algorithm to update state and episode timestep.
            isaac_gym_sum_step += self.number_of_env
            state, rewards, done = self.algo.step(self.env, self.number_of_env, state,  isaac_gym_sum_step, GAT_policy = self.GAT_policy)
            for i in range(self.number_of_env):
                env_steps[i] += 1
    ...
    def fine_tune(self, right_param, data_budget_number):
        # Time to start training.
        self.start_time = time()

        self.env.set_params(right_param)
        # Episode's timestep.
        ...
        self.algo.buffer.delete_buffer()
        ...
        while isaac_gym_sum_step < self.num_steps:
            ...
            if isaac_gym_sum_step <= data_budget_number:
                data_budget = False
            else:
                if not data_budget:
                    print('-----------------data full---------------------')
        ...
        for finetune_iter in range(1,2000):
            if self.algo.is_update(isaac_gym_sum_step):
                self.algo.update(self.writer)
