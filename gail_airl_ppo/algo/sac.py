...
class SAC(Algorithm):
    ...
    def step(self, env, number_of_env, state,  step, data_budget = False):
        """
        ...
        # if not torch.isnan(reward).sum() == 0:
        #     print('nan')
        # if not torch.isnan(next_state).sum() == 0:
        #     print('nan')
        #     for i in range(len(next_state)):
        #         if not torch.isnan(next_state[i]).sum() == 0:
        #             print(state[i])
        #             print(action[i])
        #             print(next_state[i])
        # if not torch.isnan(done).sum() == 0:
        #     print('nan')
        # if not torch.isnan(action).sum() == 0:
        #     print('action')
...
class SACExpert(SAC):
    ...
    def step(self, env, number_of_env, state):
